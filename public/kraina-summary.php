...
function invoices_init()
{
    ...
    function kraina_summary_add_scripts($hook_suffix)
    {
        ...
        wp_register_script('spa_script', plugins_url('/assets/index.js', __FILE__), array('jquery'), $ver, true);
        wp_enqueue_script('spa_script');
        ...
        wp_enqueue_style('kraina_plugin_styles_2', plugins_url('/assets/index.css', __FILE__), '', $ver);
    }
    ...
    //generalConfig
    add_action('wp_ajax_general_config', 'general_config');
        ...
        <form action="options.php" method="post">
            ...
            <div>
                <table class="kraina-section">
                    <tr>
                        <th>Tekst</th>
                        <td style="width: 100%"><input id="urlop-text" type="text" placeholder="Opis"
                                                       name="map_urlop_text"
                                                       value="<?php echo esc_attr(get_option('map_urlop_text')); ?>"
                                                       size="50" /></td>
                    </tr>
                    <tr>
                        <th>Aktywny</th>
                        <td>
                            <input name="map_urlop_checkbox" type="checkbox"
                                   value="1" <?php checked('1', get_option('map_urlop_checkbox')); ?> />
                        </td>
    ...
    function kraina_summary_render_settings_page()
    {
        ...
        $total_sell = 0;
        ?>
        <div id="react-spa-container"></div>
        <h1>Kraina Zamówienia</h1>
        <button class="button hidden" id="testowy-button">TEST</button>

        <div id="order-details-window">
            <svg id="close-window-icon" version="1.1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                 viewBox="0 0 241.171 241.171" style="enable-background:new 0 0 241.171 241.171;" xml:space="preserve">  <path
                        id="Close"
        ...
        <div class="reversed-container">
            <table id="kraina-orders-list" class="widefat">
                ...
                foreach ($orders as $order_id) {
                    ...
                    <tr order-id="<?php echo $order->get_id(); ?>"
                        class="<?php if ($order->get_status() == 'completed') echo 'green'; else echo 'red'; ?>">
                        <td><input type="checkbox" class="order-checkbox"
                                   name="name1" <?php if (($order->get_status() == 'completed') || ($order->get_meta('package_id') == '')) echo 'disabled'; ?>/>
                        </td>
                        <td class="row-title"><a
                                    href="/wp-admin/post.php?post=<?php echo $order->get_id(); ?>&action=edit"><?php echo $order->get_id(); ?></a>
                        </td>
                        <td><?php echo $order->get_date_created()->date("j M Y, G:i"); ?></td>
                        <td class="client-info">
                            <p class="hidden">ID: <span
                                        class="customer-ID"><?php echo $order->get_customer_id(); ?></span></p>
                            <p class="hidden">Nazwa: <span
                                        class="shipping-name"><?php echo $order->get_formatted_shipping_full_name(); ?></span>
                            </p>
                            <p><b>Dane:</b> <span
                                        class="shipping-first-name"><?php echo $order->get_shipping_first_name(); ?></span>
                                <span class="shipping-last-name"><?php echo $order->get_shipping_last_name(); ?></span>
                            </p>
                            <p><b>Adres:</b> <span
                                        class="shipping-address-1"><?php echo $order->get_shipping_address_1(); ?></span>
                                <span class="shipping-address-2"><?php echo $order->get_shipping_address_2(); ?></span>,
                                <span class="shipping-postcode"><?php echo $order->get_shipping_postcode(); ?></span>
                                <span class="shipping-city"><?php echo $order->get_shipping_city(); ?></span></p>
                            <p><b>Mail:</b> <span
                                        class="customer-email"><?php echo $order->get_billing_email(); ?></span></p>
                        ...
                        <td class="buttons" order-id="<?php echo $order->get_ID(); ?>">
                            ...
                            if (!in_array($order->get_status(), array('cancelled', 'pending', 'refunded'))) {
                                if (($order->get_meta('_invoice_id') == '') || ($order->get_meta('_invoice_id') == '0')) {
                                    ?><input type="button" class="button" onclick="add_invoice(this)" value="Generuj"
                                             order-id="<?php echo $order->get_ID(); ?>"><?php

                                } else {
                                    if (($order->get_status()) != 'completed') {
                                        ?>
                                        <button class="button" value="Pobierz"
                                                invoice-id="<?php echo $order->get_meta('_invoice_id'); ?>"
                                                onclick="download_invoice(this)"
                                                link="https://mariusz-pacyga.fakturownia.pl/invoices/<?php echo $order->get_meta('_invoice_id'); ?>.pdf?api_token=YHRPsXlpbZ93Flx2oDH">
                                            Pobierz
                                        </button>
                                        <input type="button" class="button btn-remove" onclick="delete_invoice(this)"
                                               value="Usuń" invoice-id="<?php echo $order->get_meta('_invoice_id'); ?>"
                                               order-id="<?php echo $order->get_ID(); ?>"><?php
                                    }
                        ...
                        <td class="see-details"><a type="button" class="button" onclick="download_order_pdf(this)"
                                                   order-id="<?php echo $order->get_ID(); ?>">Podsumowanie PDF</a>
                        </th>
                        <td class="status">
                            <?php
                        ...
                        <td class="hidden"><a type="button" class="button" value="JSON data"
                                              onclick="get_order_JSON(this)" order-id="<?php echo $order->get_ID(); ?>">Get
                                JSON data</a></td>
                    </tr>
                <?php } ?>
                </tbody>
            ...
            <div style="display: flex;">
                    ...
                    <div class="kraina-section">
                        ...
                        <table>
                            <tr>
                                <td>Od</td>
                                <td>
                                    <input id="input-orders-from" type="date" placeholder="Opis"
                                           name="kraina_orders_from"
                                           value="<?php echo esc_attr(get_option('kraina_orders_from')); ?>" />
                                </td>
                            </tr>
                            <tr>
                                <td>Do</td>
                                <td>
                                    <input id="input-orders-to" name="kraina_orders_to" type="date"
                                           value="<?php echo esc_attr(get_option('kraina_orders_to')); ?>" />
                                </td>
                            </tr>
                ...
                <div id="dispatch-orders-section" class="kraina-section">
                    ...
                    <a id="new-dispatch-button" type="button" class="button disabled-button" value="new_dispatch"
                       onclick="new_dispatch_order(this)">Utwórz zlecenie odbioru</a>
                </div>

                <div class="kraina-section">
                    <h3>DPD</h3>
                    <a id="dpd-new-dispatch" type="button" class="button disabled-button" value="dpd_new_dispatch"
                       onclick="dpd_new_dispatch(this)">Utwórz nowe zlecenie odbioru</a>
                </div>
